...
public class KaboomView extends JPanel {
	 ...
	 */
	public KaboomView(GameCode game) {
		this.game = game;
		setBackground(BACKGROUND_COLOR);
		setPreferredSize(new Dimension((int) GameCode.WIDTH, (int) GameCode.HEIGHT));
	}
	...
	protected void paintComponent(Graphics g) {
		...
		g.fillRect(((int) GameCode.BUCKET.TopLeftCorner.pX), (int)GameCode.BUCKET.TopLeftCorner.pY, 
				(int) GameCode.BUCKET.x, (int) GameCode.BUCKET.y);
		g.setColor(BOMBER_COLOR);
		g.fillRect(((int) GameCode.BOMBER.TopLeftCorner.pX), (int)GameCode.BOMBER.TopLeftCorner.pY, 
				(int) GameCode.BOMBER.x, (int) GameCode.BOMBER.y);
		

		g.setColor(BOMB_COLOR);
		for (int i = 0; i < GameCode.BOMBS.size(); i++) {
	    	 g.fillOval((int)GameCode.BOMBS.get(i).Origin.pX, (int)GameCode.BOMBS.get(i).Origin.pY, 15, 15);
	    	}
	    ...
	    g.setFont(Score);
	    g.drawString(Integer.toString(GameCode.score), 540, 60);
	    //Display difficulty
		...
		if(game.gameOver==true){
			Font Message = new Font("Serif", Font.BOLD, 50);
			    ...
			    int xstart = ((int) GameCode.WIDTH - fm.stringWidth(s)) / 2;
			    int ystart = (int) (GameCode.HEIGHT / 2);
